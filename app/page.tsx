...
const screens = [
  { type: 'cover' },
  { kicker: '09:50 / Arrival', eyebrow: 'Ten minutes before the course', title: 'You are early.\nSo is the doubt.', body: 'The room is ready. The people are not — not quite.', aside: 'A course begins before anyone starts teaching.', illustration: 'door', tone: 'paper' },
  ...
  { kicker: '09:52 / Look around', eyebrow: 'Nothing is happening yet', title: 'A chair. A mat.\nA bag by the door.', body: 'Ordinary objects. But uncertainty can make a room feel like a test.', aside: 'Tap each word. Name the room before it names you.', objects: ['chair', 'mat', 'door', 'bag'], illustration: 'room', tone: 'ink' },
  ...
  { kicker: '09:56 / Pair up', eyebrow: 'The room becomes smaller', title: 'Not in front of everyone.\nWith one other person.', body: 'A smaller social risk can create more room for attention, curiosity and practice.', aside: 'Design the conditions, not the courage.', illustration: 'chairs', tone: 'paper' },
  ...
  { kicker: '09:58 / Feedback', eyebrow: 'Specific. Kind. Useful.', quote: '“Keep that part. Try this part once more.”', body: 'Feedback points forward. It does not turn the learner into the mistake.', illustration: 'notes', tone: 'lime' },
...
export default function Home() {
  ...
  return <main className={`course-shell tone-${screen.tone ?? 'paper'}`}>
    ...
    {current === 0 ? <Cover /> : <section className="lesson-stage" aria-live="polite">
      ...
      <aside className="lesson-aside"><p className="time-code">{screen.kicker}</p>{screen.illustration ? <ObjectIllustration kind={screen.illustration} /> : screen.index ? <strong className="design-index">{screen.index}</strong> : <MinuteMark minute={current < 12 ? current - 1 : 10} />}{screen.aside && <p className="aside-note">{screen.aside}</p>}</aside>
    </section>}
